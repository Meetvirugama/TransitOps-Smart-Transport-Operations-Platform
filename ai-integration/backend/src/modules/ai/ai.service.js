 ...
 * LLMs often ignore format instructions, so we sanitize server-side.
 */
function stripMarkdown(text) {
  return text
    ...
    .replace(/^[-*]\s+/gm, '• ')             // - bullet or * bullet → • bullet
    .replace(/^\d+\.\s+/gm, '• ')            // 1. numbered list → • bullet
    .replace(/\n{3,}/g, '\n\n')              // collapse excess blank lines
    .trim();
}

/**
 * Fetch a live snapshot of the fleet from the database to use as AI context.
 */
async function getFleetContext() {
  try {
    const [vehiclesRes, tripsRes, driversRes] = await Promise.all([
    ...
    console.error('[AI Service] Failed to fetch fleet context:', err.message);
    return { vehicles: {}, trips: {}, totalDrivers: 0 };
  }
}

/**
 * Build a system prompt with live fleet context for Gemini.
 */
function buildSystemPrompt(context) {
  const vehicleSummary = Object.entries(context.vehicles)
    .map(([status, count]) => `${count} ${status}`)
...
## IMPORTANT: Use the fleet snapshot above to answer questions directly.
Do NOT say you need more data or that data is unavailable — you have the snapshot above.
If a question needs data not in the snapshot, make a reasonable inference based on what is available.

## Your Capabilities:
- Analyze fleet status and utilization using the data above
- Recommend driver-vehicle assignments
- Identify maintenance patterns and risks
- Summarize trip performance from the snapshot data
- Provide operational best practices for transport management
...
- Keep responses under 200 words
- Be concise, professional, and data-driven
- Start your answer directly — no preamble like "Based on the data..."`;
}

/**
 * Get a maintenance insight for a specific vehicle's status history.
 */
async function getMaintenanceInsight() {
  try {
    // Get vehicles with their maintenance count and last maintenance date
    const res = await dbPool.query(`
      SELECT 
        v.id,
    ...
    if (res.rows.length === 0) {
      return 'No vehicle maintenance data found in the system yet. Start logging maintenance records to get AI insights.';
    }
...
- Be direct and actionable`;
    ...
    if (err.message?.includes('API key')) {
      return '• Add your Gemini API key to backend/.env as GEMINI_API_KEY to enable AI insights.\n• Get a free key at aistudio.google.com\n• No credit card required.';
    }
    return '• Unable to generate insights at this time.\n• Ensure vehicles and maintenance records exist in the database.\n• Check backend logs for details.';
  }
}
...
/**
 * Send a message to the AI assistant with live fleet context.
 */
async function chatWithAssistant(userMessage) {
  if (!env.geminiApiKey || env.geminiApiKey === 'YOUR_GEMINI_API_KEY_HERE') {
    return {
...
module.exports = { chatWithAssistant, getMaintenanceInsight };
